...
class HyperbolicalModel(H2Projection):
    ...
    def project(self, point: H2Vector) -> Vector2:
        ...
        projected_point: Vector2 = Vector2(*view_point.hyperbolical)
        return self.projected_to_display_space(projected_point)

    def reproject(self, point: Vector2) -> H2Vector:
        gamma, beta = self.display_to_projected_space(point)
        view_point: H2Vector = H2Vector.FromHyperbolical(gamma, beta)
