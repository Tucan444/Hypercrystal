...
for i in range(lines_n - 1):
    lines_raw.append(H2Line.UltraparallelAtT(lines_raw[0], random.uniform(0, 1),
                                             random.uniform(-2, 2), 0.6, 0.6))
...
ultraparallel_line: H2Line = H2Line(H2Vector(), H2Vector.FromHyperpolar(0.5, 0.2))
lines.append(ultraparallel_line.approximate(30))
    ...
    for event in pygame.event.get():
        if event.type == MOUSEBUTTONDOWN:
            ...
            if reprojected_mouse_pos is not None:
                ...
                ultraparallel_line = H2Line.UltraparallelToPoint(lines_raw[0], reprojected_mouse_pos)

                if tangent_line is not None:
                    lines[-2] = tangent_line.approximate(len(lines[-2].points))

                lines[-1] = ultraparallel_line.approximate(len(lines[-1].points))
