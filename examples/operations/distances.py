...
from hypercrystal.operators import Distances
...
while alive:
    ...
    for event in pygame.event.get():
        if event.type == MOUSEBUTTONDOWN:
            ...
            reprojected_position: H2Vector = projection.reproject(mouse_pos)
            if reprojected_position is not None:
                distance: float = 1000000

                for line in lines_raw:
                    distance = min(distance, Distances.PointLine(reprojected_position, line))

                for circle in circles_raw:
                    distance = min(distance, Distances.PointCircle(reprojected_position, circle))

                print(f"Distance to closest object in {distance}.")
